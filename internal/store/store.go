...
func (s *Store) CreateSyncPair(pair *SyncPair) error {
	result, err := s.db.Exec(
		`INSERT INTO sync_pairs (name, local_path, remote_path, provider, mode, direction, enabled, schedule)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (s *Store) GetSyncPair(id int64) (*SyncPair, error) {
	pair := &SyncPair{}
	err := s.db.QueryRow(
		`SELECT id, name, local_path, remote_path, provider, mode, direction, enabled, schedule, created_at, updated_at
		 FROM sync_pairs WHERE id = ?`, id,
	).Scan(&pair.ID, &pair.Name, &pair.LocalPath, &pair.RemotePath, &pair.Provider,
		&pair.Mode, &pair.Direction, &pair.Enabled, &pair.Schedule, &pair.CreatedAt, &pair.UpdatedAt)
...
func (s *Store) ListSyncPairs() ([]*SyncPair, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&pair.ID, &pair.Name, &pair.LocalPath, &pair.RemotePath, &pair.Provider,
			&pair.Mode, &pair.Direction, &pair.Enabled, &pair.Schedule, &pair.CreatedAt, &pair.UpdatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) UpdateSyncPair(pair *SyncPair) error {
	_, err := s.db.Exec(
		`UPDATE sync_pairs SET local_path=?, remote_path=?, provider=?, mode=?, direction=?, enabled=?, schedule=?, updated_at=CURRENT_TIMESTAMP
		 WHERE id = ?`,
		pair.LocalPath, pair.RemotePath, pair.Provider, pair.Mode, pair.Direction, pair.Enabled, pair.Schedule, pair.ID)
	return err
}
